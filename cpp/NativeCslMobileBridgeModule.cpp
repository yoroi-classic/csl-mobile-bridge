...
CslMobileBridge::CslMobileBridge(Runtime& rt) : runtime_(rt) {
...
Value CslMobileBridge::get(Runtime& rt, const PropNameID& name) {
  auto it = methods_.find(name.utf8(rt));
  return it != methods_.end() ? Value(rt, it->second) : Value::undefined();
}

void CslMobileBridge::install(Runtime& rt) {
